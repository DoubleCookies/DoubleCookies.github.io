...
function FavoriteProjects() {
    ...
    const data = [
        {
            ...
            description: (
                <div>
                    {'ne of the old projects that was created for fun, and in the end it sunk into the soul seriously and for long.' +
                        ' Любовь к лабиринтам + визуализация + всякие фичи = <span role="img" aria-label="heart">❤</span>'}
                </div>
            ),
    ...
    const dataItems = data.map((item) => (
        <li key={item.title}>
            <a href={item.link} style={{ marginRight: 4 }}>
                {item.title}.
            </a>
    ...
    return (
        <div>
            ...
            <ul className="what-list">{dataItems}</ul>
        </div>
    );
}
